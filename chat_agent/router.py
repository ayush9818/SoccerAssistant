...
class RouterComponent(QueryComponent):
    ...
    def __init__(
        ...
        self._summarizer = TreeSummarize(
            llm=self._llm,
    ...
    def _run_component(self, **kwargs: Any) -> Any:
        ...
        if len(sel_output.selections) > 1:
            responses = []
            for i, sel in enumerate(sel_output.selections):
                engine_ind = sel.index
                log_str = (
                    f"Selecting query engine {engine_ind}: " f"{sel.reason}."
                )
                if self.verbose:
                    print_text(log_str + "\n", color="pink")

                component = self.components[engine_ind]
                responses.append(str(component.run_component(input= kwargs["query"])))

            if len(responses) > 1:
                summary = self._summarizer.get_response(kwargs["query"], responses)
                return summary
            else:
                final_response = responses[0]
                return final_response
        elif len(sel_output.selections) == 1:
            ...
            return component.run_component(input= kwargs["query"])
        else:
